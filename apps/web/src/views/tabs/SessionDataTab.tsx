...
export const SessionDataTab: React.FC<SessionDataTabProps> = ({ patient }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Machine Prescription and Filter Health */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Card>
          <h3 className="text-sm font-semibold text-renal-text mb-4">Prescription Details</h3>
          <div className="grid grid-cols-2 gap-y-4">
            <div>
              <div className="text-xs text-renal-muted mb-1">Technique</div>
              <div className="text-sm font-medium text-renal-text">{sessions[0]?.technique || 'HD'}</div>
            </div>
            <div>
              <div className="text-xs text-renal-muted mb-1">Dialyzer</div>
              <div className="text-sm font-medium text-renal-text">{sessions[0]?.dialyzerType || 'High Flux'}</div>
            </div>
            <div>
              <div className="text-xs text-renal-muted mb-1">Dialysate Flow</div>
              <div className="text-sm font-medium text-renal-text">{sessions[0]?.dialysateFlowRate || 500} mL/min</div>
            </div>
            <div>
              <div className="text-xs text-renal-muted mb-1">Dialysate Temp</div>
              <div className="text-sm font-medium text-renal-text">{sessions[0]?.dialysateTemp || 36.5}°C</div>
            </div>
          </div>
        </Card>

        <Card>
          <h3 className="text-sm font-semibold text-renal-text mb-4">Filter & Line Health</h3>
          <div className="grid grid-cols-2 gap-y-4">
            <div>
              <div className="text-xs text-renal-muted mb-1">TMP</div>
              <div className={cn(
                "text-sm font-medium",
                (sessions[0]?.tmp || 0) > 200 ? "text-rs-red" : "text-renal-text"
              )}>
                {sessions[0]?.tmp || 120} mmHg
              </div>
            </div>
            <div>
              <div className="text-xs text-renal-muted mb-1">Venous Pressure</div>
              <div className="text-sm font-medium text-renal-text">{sessions[0]?.vp || 150} mmHg</div>
            </div>
            <div>
              <div className="text-xs text-renal-muted mb-1">Arterial Pressure</div>
              <div className="text-sm font-medium text-renal-text">{sessions[0]?.arterialPressure || -150} mmHg</div>
            </div>
            <div>
              <div className="text-xs text-renal-muted mb-1">Blood Flow (BFR)</div>
              <div className="text-sm font-medium text-renal-text">{sessions[0]?.bfr || 300} mL/min</div>
            </div>
          </div>
          {(sessions[0]?.tmp || 0) > 180 && (
            <div className="mt-4 p-2 bg-rs-amber/10 border border-rs-amber/30 rounded text-[10px] text-rs-amber">
              ⚠️ Filter clotting risk: TMP is trending high.
            </div>
          )}
        </Card>
      </div>
      ...
      <Card>
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-sm font-semibold text-renal-text">Session Event Log</h3>
